...
function RiderSignIn({ setPage }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (user.role !== 'rider') {
        return setError('This account is not a rider account.')
      }
  ...
  return (
    <div style={{backgroundColor: '#EAEDED', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '40px'}}>
      ...
      <div style={{backgroundColor: 'white', padding: '30px', borderRadius: '8px', width: '100%', maxWidth: '400px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)'}}>
        ...
        <h2 style={{fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '6px', color: '#131921'}}>Rider Sign In</h2>
        <p style={{color: '#555', fontSize: '0.9rem', marginBottom: '20px'}}>Sign in to view your deliveries</p>
        ...
        <p style={{textAlign: 'center', color: '#555', fontSize: '0.9rem'}}>
          New rider? <span onClick={() => setPage('riderregister')} style={{color: '#006B3F', cursor: 'pointer', fontWeight: 'bold'}}>Join as a Rider</span>
        </p>
